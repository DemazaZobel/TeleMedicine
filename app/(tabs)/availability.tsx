...
export default function AvailabilityScreen() {
  ...
  const handleDelete = (id: string | number) => {
    if (Platform.OS === 'web') {
        if (confirm(t("doctor:deleteBlockConfirm"))) {
            deleteAvailabilityRule(id);
        }
        return;
    }
    Alert.alert(t("doctor:removeHours"), t("doctor:deleteSlotConfirm"), [
      ...
      <View style={styles.contentWrapper}>
        <View style={styles.topActions}>
            <View style={styles.viewToggle}>
                <TouchableOpacity onPress={() => setViewMode('list')} style={[styles.toggleBtn, viewMode === 'list' && styles.toggleBtnActive]}>
                    ...
                    <Text style={[styles.toggleText, viewMode === 'list' && styles.toggleTextActive]}>{t("common:list")}</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => setViewMode('calendar')} style={[styles.toggleBtn, viewMode === 'calendar' && styles.toggleBtnActive]}>
                    ...
                    <Text style={[styles.toggleText, viewMode === 'calendar' && styles.toggleTextActive]}>{t("appointment:calendar")}</Text>
                </TouchableOpacity>
            </View>
            <Button
              title={t("doctor:addAvailability")}
              variant="primary"
              size="sm"
        ...
        <View style={styles.subHeader}>
            ...
            <View style={styles.centerStat}>
                ...
                <Text style={styles.statLabel}>{t("doctor:appointmentsTodayCount")}</Text>
            </View>
            ...
            <View style={styles.rightGroup}>
                <TouchableOpacity style={[styles.iconAction, showFilters && styles.iconActionActive]} onPress={() => setShowFilters(!showFilters)}>
                    ...
                    <Text style={styles.iconActionText}>{t("patient:filter")}</Text>
                </TouchableOpacity>
            </View>
        </View>
        ...
        {viewMode === 'calendar' ? (
            ...
            <View style={styles.listViewContainer}>
                ...
                {listItems.length === 0 ? (
                    <EmptyState icon="calendar-outline" title={t("common:noItemsFound")} description={t("doctor:noSlotsForDate")} actionLabel="Add Hours" onAction={() => setShowAddModal(true)} />
                ) : (
